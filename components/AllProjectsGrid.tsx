...
interface AllProjectsGridProps {
  ...
  isPage: boolean; // Is this a standalone page or a homepage section?
}

const AllProjectsGrid: React.FC<AllProjectsGridProps> = ({ onVideoSelect, isPage }) => {
  const [ref, isVisible] = useIntersectionObserver({ threshold: 0.1 });
  
  const TitleTag = isPage ? 'h1' : 'h2';
  const sectionPadding = isPage ? 'pt-40 pb-20 md:pb-32 min-h-screen' : 'py-20 md:py-32';
  const titleSize = isPage ? 'text-5xl md:text-7xl' : 'text-4xl md:text-5xl';


  return (
    <section id="all-projects" className={`${sectionPadding} bg-dark-secondary`}>
      <div className="container mx-auto px-6" ref={ref}>
        {isVisible && (
            <>
                <TitleTag className={`${titleSize} font-black text-center uppercase animate-fadeInUp`}>
                  Our <span className="text-ferrari-red">Portfolio</span>
                </TitleTag>
